...
const TestUpload = ({ onSuccess }) => {
  ...
  const [emailError, setEmailError] = useState("");
  const [contactNoError, setContactNoError] = useState("");
  const [parentNoError, setParentNoError] = useState(""); // New state for parentNo validation
  ...
  // Parent number validation (same as contactNo)
  const validateParentNo = (parentNo) => {
    return /^\d{10}$/.test(parentNo);
  };
  ...
  // Handle parent number input change
  const handleParentNoChange = (e) => {
    const value = e.target.value;
    setParentNo(value);
    if (!validateParentNo(value)) {
      setParentNoError("Parent's contact number must be exactly 10 digits.");
    } else {
      setParentNoError("");
    }
  };
  ...
  const isFormValid =
    ...
    validateEmail(email) &&
    validateContactNo(contactNo) &&
    validateParentNo(parentNo); // Include parentNo validation
